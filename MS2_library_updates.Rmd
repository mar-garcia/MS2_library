...
db$mz <- NA
for(i in 1:nrow(db)){
  if(db$adduct[i] %in% adducts()[, 1]){
    if(grepl("D", db$formula[i])){
      db$mz[i] <- unlist(CompoundDb::mass2mz(
        getMolecule(db$formula[i])$exactmass, db$adduct[i]))
    } else {
      db$mz[i] <- unlist(CompoundDb::mass2mz(
        MonoisotopicMass(db$formula[i], formula = ListFormula(db$formula[i])), 
                                             db$adduct[i]))
    }
    ...
    for(j in 1:length(losses)){
      if(grepl("D", losses[j])){
        ...
        c_mz <- c_mz - MonoisotopicMass(
          losses[j], formula = ListFormula(losses[j]))
      }
    }
    db$mz[i] <- c_mz
  }
